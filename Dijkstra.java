...
public final class Dijkstra {
   ...
   */
  public static <Integer> Map<Integer, Double> shortestPaths(DirectedGraph<Integer> graph, Integer source) {
     ...
     */
    for (Integer node : graph) entries.put(node, pq.enqueue(node, Double.POSITIVE_INFINITY));
    ...
    while (!pq.isEmpty()) {
      ...
      for (Map.Entry<Integer, Double> arc : graph.edgesFrom(curr.getValue()).entrySet()) {
        ...
        if (pathCost < dest.getPriority()) pq.decreaseKey(dest, pathCost);
      }
    }
