 ...
 *
 * @param Integer The type of elements to store in the heap.
 * @author Keith Schwarz (htiek@cs.stanford.edu)
 */
public final class FibonacciHeap<Integer> {
   ...
   * private.
   */
  public static final class Entry<Integer> {
    private int mDegree = 0; // Number of children
    private boolean mIsMarked = false; // Whether this node is marked

    private Entry<Integer> mNext; // Next and previous elements in the list
    private Entry<Integer> mPrev;

    private Entry<Integer> mParent; // Parent in the tree, if any.

    private Entry<Integer> mChild; // Child node, if any.

    private Integer mElem; // Element being stored here
     ...
     */
    private Entry(Integer elem, double priority) {
  ...
  private Entry<Integer> mMin = null;

  /* Cached size of the heap, so we don't have to recompute this explicitly. */
  private int mSize = 0;

  /**
   * Inserts the specified element into the Fibonacci heap with the specified priority. Its priority
   * must be a valid double, so you cannot set the priority to NaN.
   *
   * @param value The value to insert.
   * @param priority Its priority, which must be valid.
   * @return An Entry representing that element in the tree.
   */
  public Entry<Integer> enqueue(Integer value, double priority) {
    checkPriority(priority);

    /* Create the entry object, which is a circularly-linked list of length
     * one.
     */
    Entry<Integer> result = new Entry<Integer>(value, priority);

    /* Merge this singleton list with the tree list. */
    this.mMin = mergeLists(this.mMin, result);
   ...
   */
  public static <Integer> FibonacciHeap<Integer> merge(FibonacciHeap<Integer> one, FibonacciHeap<Integer> two) {
    /* Create a new FibonacciHeap to hold the result. */
    FibonacciHeap<Integer> result = new FibonacciHeap<Integer>();
   ...
   * @throws NoSuchElementException If the heap is empty.
   */
  public Entry<Integer> dequeueMin() {
     ...
     */
    List<Entry<Integer>> treeTable = new ArrayList<Entry<Integer>>();
     ...
     */
    List<Entry<Integer>> toVisit = new ArrayList<Entry<Integer>>();
     ...
     * of that list.
     */
    for (Entry<Integer> curr = mMin; toVisit.isEmpty() || toVisit.get(0) != curr; curr = curr.mNext)
      toVisit.add(curr);

    /* Traverse this list and perform the appropriate unioning steps. */
    for (Entry<Integer> curr : toVisit) {
      ...
      while (true) {
        ...
        Entry<Integer> other = treeTable.get(curr.mDegree);
         ...
         */
        Entry<Integer> min = (other.mPriority < curr.mPriority) ? other : curr;
        Entry<Integer> max = (other.mPriority < curr.mPriority) ? curr : other;
  ...
  /**
   * Decreases the key of the specified element to the new priority. If the new priority is greater
   * than the old priority, this function throws an IllegalArgumentException. The new priority must
   * be a finite double, so you cannot set the priority to be NaN, or +/- infinity. Doing so also
   * throws an IllegalArgumentException.
   *
   * <p>It is assumed that the entry belongs in this heap. For efficiency reasons, this is not
   * checked at runtime.
   *
   ...
   * @throws IllegalArgumentException If the new priority exceeds the old priority, or if the
   *     argument is not a finite double.
   */
  public void decreaseKey(Entry<Integer> entry, double newPriority) {
   ...
   */
  public void delete(Entry<Integer> entry) {
  ...
  /**
   * Utility function which, given a user-specified priority, checks whether it's a valid double and
   ...
   */
  private static <Integer> Entry<Integer> mergeLists(Entry<Integer> one, Entry<Integer> two) {
    ...
    if (one == null && two == null) { // Both null, resulting list is null.
       ...
       *
       */
      Entry<Integer> oneNext = one.mNext; // Cache this since we're about to overwrite it.
      one.mNext = two.mNext;
      one.mNext.mPrev = one;
      two.mNext = oneNext;
      two.mNext.mPrev = two;
  ...
  /**
   * Decreases the key of a node in the tree without doing any checking to ensure that the new
   * priority is valid.
   *
   * @param entry The node whose key should be decreased.
   * @param priority The node's new priority.
   */
  private void decreaseKeyUnchecked(Entry<Integer> entry, double priority) {
   ...
   */
  private void cutNode(Entry<Integer> entry) {
